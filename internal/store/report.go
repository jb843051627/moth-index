...
func (s *ReportStore) BatchSummary(ctx context.Context, batchID int64) (model.BatchSummary, error) {
	...
	if err := s.db.QueryRow(ctx, `SELECT COUNT(*) FROM signals WHERE batch_id=? AND active=0 AND severity=?`, batchID, model.SeverityBlocker).Scan(&summary.Blockers); err != nil {
