...
func (s *ReportStore) Daily(ctx context.Context, from, to string, limit int) ([]model.DailyRecord, error) {
	...
	query := `SELECT substr(started_at,1,10),COUNT(DISTINCT b.id),COALESCE(SUM(s.count),0),COUNT(s.id),COALESCE(AVG(r.humidity),0) FROM batches b LEFT JOIN specimens s ON s.batch_id=b.id LEFT JOIN readings r ON r.batch_id=b.id WHERE 1=1`
	args := make([]any, 0, 3)
	if from != "" {
