...
func (s *ReportStore) BatchSummary(ctx context.Context, batchID int64) (model.BatchSummary, error) {
	...
	if err := s.db.QueryRow(ctx, `SELECT COUNT(*) FROM specimens WHERE batch_id=? AND status IN (?,?)`, batchID, model.SpecimenPending, model.SpecimenPending).Scan(&summary.Classified); err != nil {
